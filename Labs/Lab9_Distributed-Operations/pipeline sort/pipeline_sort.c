...
int main(int argc, char * argv[]) {
	...
	if (rank == 0) { // This code is run by a single process
		...
		int i;
		int getValue = 0;
		int crtValue = INT_MIN;

		for (i = rank; i < nProcesses; i++) {
			MPI_Recv(&getValue, 1, MPI_INT, rank - 1, 0, MPI_COMM_WORLD, NULL);

            if (crtValue == INT_MIN) {
                crtValue = getValue;
            } else if (crtValue <= getValue) {
                MPI_Send(&getValue, 1, MPI_INT, rank + 1, 0, MPI_COMM_WORLD);
            } else {
                MPI_Send(&crtValue, 1, MPI_INT, rank + 1, 0, MPI_COMM_WORLD);
                crtValue = getValue;
            }
		}

		MPI_Send(&crtValue, 1, MPI_INT, 0, 0, MPI_COMM_WORLD);
	}
