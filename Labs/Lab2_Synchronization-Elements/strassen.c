#include <stdio.h>
#include <stdlib.h>
#include <pthread.h>

#define NUM_THREADS 7

pthread_barrier_t barrier;

int N;
int **a;
int **b;
int **c;
int **M1;
int **AUXM11;
int **AUXM12;
int **M2;
int **AUXM21;
int **M3;
int **AUXM31;
int **M4;
int **AUXM41;
int **M5;
int **AUXM51;
int **M6;
int **AUXM61;
int **AUXM62;
int **M7;
int **AUXM71;
int **AUXM72;


void get_args(int argc, char **argv)
{
	if (argc < 2)
	{
		printf("Numar insuficient de parametri: ./program N\n");
		exit(1);
	}

	N = atoi(argv[1]);
}

int **alloc_matrix(int n, int m)
{
	int **matrix;
	matrix = malloc(sizeof(int *) * n);
	if (matrix == NULL)
	{
		printf("Eroare la malloc");
		exit(1);
	}

	int i;
	for (i = 0; i < N; i++)
	{
		matrix[i] = malloc(sizeof(int) * N);
		if (matrix[i] == NULL)
		{
			printf("Eroare la malloc");
			exit(1);
		}
	}

	return (matrix);
}

void init()
{
	int i, j;
	a = alloc_matrix(N, N);
	b = alloc_matrix(N, N);
	c = alloc_matrix(N, N);

	M1 = alloc_matrix(N, N);
	AUXM11 = alloc_matrix(N, N);
	AUXM12 = alloc_matrix(N, N);

	M2 = alloc_matrix(N, N);
	AUXM21 = alloc_matrix(N, N);

	M3 = alloc_matrix(N, N);
	AUXM31 = alloc_matrix(N, N);

	M4 = alloc_matrix(N, N);
	AUXM41 = alloc_matrix(N, N);

	M5 = alloc_matrix(N, N);
	AUXM51 = alloc_matrix(N, N);

	M6 = alloc_matrix(N, N);
	AUXM61 = alloc_matrix(N, N);
	AUXM62 = alloc_matrix(N, N);

	M7 = alloc_matrix(N, N);
	AUXM71 = alloc_matrix(N, N);
	AUXM72 = alloc_matrix(N, N);

	for (i = 0; i < N; i++)
	{
		for (j = 0; j < N; j++)
		{
			c[i][j] = 0;

			M1[i][j] = 0;
			AUXM11[i][j] = 0;
			AUXM12[i][j] = 0;

			M2[i][j] = 0;
			AUXM21[i][j] = 0;

			M3[i][j] = 0;
			AUXM31[i][j] = 0;

			M4[i][j] = 0;
			AUXM41[i][j] = 0;

			M5[i][j] = 0;
			AUXM51[i][j] = 0;

			M6[i][j] = 0;
			AUXM61[i][j] = 0;
			AUXM62[i][j] = 0;

			M7[i][j] = 0;
			AUXM71[i][j] = 0;
			AUXM72[i][j] = 0;

			if (i <= j)
			{
				a[i][j] = 1;
				b[i][j] = 1;
			}
			else
			{
				a[i][j] = 0;
				b[i][j] = 0;
			}
		}
	}
}

void print(int **mat)
{
	int i, j;
	for (i = 0; i < N; i++)
	{
		for (j = 0; j < N; j++)
		{
			printf("%i\t", mat[i][j]);
		}
		printf("\n");
	}
}

void mul_matrix(int **C, int startCi, int startCj, int **A, int startAi, int startAj, int **B, int startBi, int startBj)
{
	// NU paralelizati aceasta functie
	int i, j, k;
	for (i = 0; i < N / 2; i++)
		for (j = 0; j < N / 2; j++)
			for (k = 0; k < N / 2; k++)
				C[startCi + i][startCj + j] += A[startAi + i][startAj + k] * B[startBi + k][startBj + j];
}

void add_matrix(int **C, int startCi, int startCj, int **A, int startAi, int startAj, int **B, int startBi, int startBj)
{
	// NU paralelizati aceasta functie
	int i, j;
	for (i = 0; i < N / 2; i++)
		for (j = 0; j < N / 2; j++)
			C[startCi + i][startCj + j] = A[startAi + i][startAj + j] + B[startBi + i][startBj + j];
}

void sub_matrix(int **C, int startCi, int startCj, int **A, int startAi, int startAj, int **B, int startBi, int startBj)
{
	// NU paralelizati aceasta functie
	int i, j;
	for (i = 0; i < N / 2; i++)
		for (j = 0; j < N / 2; j++)
			C[startCi + i][startCj + j] = A[startAi + i][startAj + j] - B[startBi + i][startBj + j];
}

void strassen_seq()
{
	// calculul matricii M1
	add_matrix(AUXM11, 0, 0, a, 0, 0, a, N / 2, N / 2);
	add_matrix(AUXM12, 0, 0, b, 0, 0, b, N / 2, N / 2);
	mul_matrix(M1, 0, 0, AUXM11, 0, 0, AUXM12, 0, 0);

	// calculul matricii M2
	add_matrix(AUXM21, 0, 0, a, N / 2, 0, a, N / 2, N / 2);
	mul_matrix(M2, 0, 0, AUXM21, 0, 0, b, 0, 0);

	// calculul matricii M3
	sub_matrix(AUXM31, 0, 0, b, 0, N / 2, b, N / 2, N / 2);
	mul_matrix(M3, 0, 0, a, 0, 0, AUXM31, 0, 0);

	// calculul matricii M4
	sub_matrix(AUXM41, 0, 0, b, N / 2, 0, b, 0, 0);
	mul_matrix(M4, 0, 0, a, N / 2, N / 2, AUXM41, 0, 0);

	// calculul matricii M5
	add_matrix(AUXM51, 0, 0, a, 0, 0, a, 0, N / 2);
	mul_matrix(M5, 0, 0, AUXM51, 0, 0, b, N / 2, N / 2);

	// calculul matricii M6
	sub_matrix(AUXM61, 0, 0, a, N / 2, 0, a, 0, 0);
	add_matrix(AUXM62, 0, 0, b, 0, 0, b, 0, N / 2);
	mul_matrix(M6, 0, 0, AUXM61, 0, 0, AUXM62, 0, 0);

	// calculul matricii M7
	sub_matrix(AUXM71, 0, 0, a, 0, N / 2, a, N / 2, N / 2);
	add_matrix(AUXM72, 0, 0, b, N / 2, 0, b, N / 2, N / 2);
	mul_matrix(M7, 0, 0, AUXM71, 0, 0, AUXM72, 0, 0);

	// calculul submatricii C1,1
	add_matrix(c, 0, 0, M1, 0, 0, M4, 0, 0);
	sub_matrix(c, 0, 0, c, 0, 0, M5, 0, 0);
	add_matrix(c, 0, 0, c, 0, 0, M7, 0, 0);

	// calculul submatricii C1,2
	add_matrix(c, 0, N / 2, M3, 0, 0, M5, 0, 0);

	// calculul submatricii C2,1
	add_matrix(c, N / 2, 0, M2, 0, 0, M4, 0, 0);

	// calculul submatricii C2,2
	sub_matrix(c, N / 2, N / 2, M1, 0, 0, M2, 0, 0);
	add_matrix(c, N / 2, N / 2, c, N / 2, N / 2, M3, 0, 0);
	add_matrix(c, N / 2, N / 2, c, N / 2, N / 2, M6, 0, 0);
}

void put_barrier(int tid) {
	int b_res = pthread_barrier_wait(&barrier);
	if (b_res == PTHREAD_BARRIER_SERIAL_THREAD)
	{
		printf("barrier up -> ");
	}
	printf("res: %d - thread: %d\n", b_res, tid);
}

void *compute_M1(void *arg)
{
	int thread_id = *(int *)arg;
	add_matrix(AUXM11, 0, 0, a, 0, 0, a, N / 2, N / 2);
	add_matrix(AUXM12, 0, 0, b, 0, 0, b, N / 2, N / 2);
	mul_matrix(M1, 0, 0, AUXM11, 0, 0, AUXM12, 0, 0);

	put_barrier(thread_id);
	pthread_exit(NULL);
}

void *compute_M2(void *arg)
{
	int thread_id = *(int *)arg;
	add_matrix(AUXM21, 0, 0, a, N / 2, 0, a, N / 2, N / 2);
	mul_matrix(M2, 0, 0, AUXM21, 0, 0, b, 0, 0);
	
	put_barrier(thread_id);
	pthread_exit(NULL);
}

void *compute_M3(void *arg)
{
	int thread_id = *(int *)arg;
	sub_matrix(AUXM31, 0, 0, b, 0, N / 2, b, N / 2, N / 2);
	mul_matrix(M3, 0, 0, a, 0, 0, AUXM31, 0, 0);

	put_barrier(thread_id);	
	pthread_exit(NULL);
}

void *compute_M4(void *arg)
{
	int thread_id = *(int *)arg;
	sub_matrix(AUXM41, 0, 0, b, N / 2, 0, b, 0, 0);
	mul_matrix(M4, 0, 0, a, N / 2, N / 2, AUXM41, 0, 0);

	put_barrier(thread_id);
	pthread_exit(NULL);
}

void *compute_M5(void *arg)
{
	int thread_id = *(int *)arg;
	add_matrix(AUXM51, 0, 0, a, 0, 0, a, 0, N / 2);
	mul_matrix(M5, 0, 0, AUXM51, 0, 0, b, N / 2, N / 2);

	put_barrier(thread_id);
	pthread_exit(NULL);
}

void *compute_M6(void *arg)
{
	int thread_id = *(int *)arg;

	sub_matrix(AUXM61, 0, 0, a, N / 2, 0, a, 0, 0);
	add_matrix(AUXM62, 0, 0, b, 0, 0, b, 0, N / 2);
	mul_matrix(M6, 0, 0, AUXM61, 0, 0, AUXM62, 0, 0);
	
	put_barrier(thread_id);
	pthread_exit(NULL);
}

void *compute_M7(void *arg)
{
	int thread_id = *(int *)arg;

	sub_matrix(AUXM71, 0, 0, a, 0, N / 2, a, N / 2, N / 2);
	add_matrix(AUXM72, 0, 0, b, N / 2, 0, b, N / 2, N / 2);
	mul_matrix(M7, 0, 0, AUXM71, 0, 0, AUXM72, 0, 0);
	
	put_barrier(thread_id);
	pthread_exit(NULL);
}

void *compute_C11(void *arg)
{
	add_matrix(c, 0, 0, M1, 0, 0, M4, 0, 0);
	sub_matrix(c, 0, 0, c, 0, 0, M5, 0, 0);
	add_matrix(c, 0, 0, c, 0, 0, M7, 0, 0);
	
	pthread_exit(NULL);
}

void *compute_C12(void *arg)
{
	add_matrix(c, 0, N / 2, M3, 0, 0, M5, 0, 0);
	pthread_exit(NULL);
}

void *compute_C21(void *arg)
{
	add_matrix(c, N / 2, 0, M2, 0, 0, M4, 0, 0);
	pthread_exit(NULL);
}

void *compute_C22(void *arg)
{
	sub_matrix(c, N / 2, N / 2, M1, 0, 0, M2, 0, 0);
	add_matrix(c, N / 2, N / 2, c, N / 2, N / 2, M3, 0, 0);
	add_matrix(c, N / 2, N / 2, c, N / 2, N / 2, M6, 0, 0);
	pthread_exit(NULL);
}

void check_thread_creation(int res, int tid) {
	if (res) {
		printf("Eroare la crearea thread-ului %d\n", tid);
		exit(-1);
	}
}

void check_thread_join(int res, int tid) {
	if (res) {
		printf("Eroare la asteptarea thread-ului %d\n", tid);
		exit(-1);
	}
}

int main(int argc, char *argv[])
{
	pthread_t threads[NUM_THREADS];
	int arguments[NUM_THREADS];
	int res;

	get_args(argc, argv);
	init();
	// strassen_seq();

	pthread_barrier_init(&barrier, NULL, NUM_THREADS);

	for (int tid = 0; tid < NUM_THREADS; tid++) {
		arguments[tid] = tid;

		if (tid == 0) {
			res = pthread_create(&threads[tid], NULL, compute_M1, &arguments[tid]);
		} else if (tid == 1) {
			res = pthread_create(&threads[tid], NULL, compute_M2, &arguments[tid]);
		} else if (tid == 2) {
			res = pthread_create(&threads[tid], NULL, compute_M3, &arguments[tid]);
		} else if (tid == 3) {
			res = pthread_create(&threads[tid], NULL, compute_M4, &arguments[tid]);
		} else if (tid == 4) {
			res = pthread_create(&threads[tid], NULL, compute_M5, &arguments[tid]);
		} else if (tid == 5) {
			res = pthread_create(&threads[tid], NULL, compute_M6, &arguments[tid]);
		} else if (tid == 6) {
			res = pthread_create(&threads[tid], NULL, compute_M7, &arguments[tid]);
		}

		check_thread_creation(res, tid);
	}

	for (int tid = 0; tid < 4; tid++) {
		arguments[tid] = tid;

		if (tid == 0) {
			res = pthread_create(&threads[tid], NULL, compute_C11, &arguments[tid]);
		} else if (tid == 1) {
			res = pthread_create(&threads[tid], NULL, compute_C12, &arguments[tid]);
		} else if (tid == 2) {
			res = pthread_create(&threads[tid], NULL, compute_C21, &arguments[tid]);
		} else if (tid == 3) {
			res = pthread_create(&threads[tid], NULL, compute_C22, &arguments[tid]);
		} 

		check_thread_creation(res, tid);
	}

	void *status;
	for (int tid = 0; tid < NUM_THREADS; tid++) {
		res = pthread_join(threads[tid], &status);
		check_thread_join(res, tid);
	}

	pthread_barrier_destroy(&barrier);
	print(c);

	return 0;
}
